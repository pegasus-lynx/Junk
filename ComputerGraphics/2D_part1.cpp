...
/*Multiplying two matrix and storing the results back in 2nd matrix*/ 
...
/*Shearing*/
void shear2D(double shx,double shy,double x,double y)
{
	Matrix3x3 matShear;
...
// 	matrixPreMultiply(matShear,matComposite);	
// }
/*Ttnasforming to new coordinates*/
...
void display()
{
	...
	{
		double tx, ty;
		cout << "Enter translation matrix values tx and ty:" << endl;
		cin >> tx >> ty;
		translate2D(tx,ty);
	...
	{
		double shx,shy;
		cout << "Enter shear values shx and shy" << endl;
		cin >> shx >> shy;
		shear2D(shx,shy,0,0);
		transformVertex(n,vertex);
	}
	...
	{
		double sx,sy;
		cout << "Enter scale factor sx and sy"<< endl;
		cin >> sx >> sy;
		scale2D(sx,sy,0,0);
		transformVertex(n,vertex);
	}
